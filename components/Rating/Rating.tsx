...
const Rating = ({isEditable = false, rating, setRating, children, className, ...props}: RatingProps): JSX.Element => {
    const [ratingArray, setRatingArray] = useState<JSX.Element[]>(new Array(5).fill(<></>));
    useEffect(() => {
        constructRating(rating);
    }, [rating]);

    const changeDisplay = (inputRating: number) => {
        if (!isEditable) {
            return;
        }
        constructRating(inputRating);
    };



    const constructRating = (currentRating: number) => {
        const updatedArray = ratingArray.map((rRating: JSX.Element, index: number) => {
            return (
                <svg onClick={() => (index + 1)}
